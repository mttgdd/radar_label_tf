#{CITYSCAPEi Copyright 2018 The TensorFlow Authors All Rights Reserved.
#
...
"""
import os.path,sys
sys.path.insert(0,'/data/prannay/models/research/')
import time
import numpy as np
...
def _process_batch(sess, original_images, semantic_predictions, image_names,
  ...
  for i in range(num_image):
    ...
    image_name = image_names[i].decode("utf-8")[:-4]
    print(image_name)
    ...
    save_annotation.save_annotation(
        original_image, save_dir, _IMAGE_FORMAT % (image_name),
        add_colormap=False)
    ...
    if FLAGS.also_save_raw_predictions:
      ...
      if train_id_to_eval_id is not None:
        crop_semantic_prediction = _convert_train_id_to_eval_id(
            ...
            train_id_to_eval_id)
      save_annotation.save_annotation(
          crop_semantic_prediction, raw_save_dir, _RAW_FORMAT % (image_name),
          add_colormap=False)
